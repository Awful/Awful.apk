...
public class PullToRefreshAttacher implements View.OnTouchListener {
    ...
    private boolean mPullFromBottom = false;
    ...
    /**
     * @return true if this PullToRefresh is currently pulled from bottom (defaults to <code>false</code>)
     */
    public boolean isPulledFromBottom() {
        return mPullFromBottom;
    }
    ...
    /**
     * Allows the enable/disable of pulling from the bottom instead of from the top. If disabled
     * when refreshing then the UI is automatically reset.
     *
     * @param enabled - Whether this PullToRefreshAttacher is pulling from the bottom.
     */
    public void setPullFromBottom(boolean enabled) {
        mPullFromBottom = enabled;
        resetTouch();
        if (!enabled && mIsRefreshing) {
            reset(false);
        }
    }
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE: {
                ...
                if (!mIsHandlingTouchEvent) {
                    if (canRefresh(true, params.onRefreshListener) && (isPulledFromBottom() ? params.viewDelegate.isScrolledToBottom(view) : params.viewDelegate.isScrolledToTop(view))) {
                ...
                if (!mIsBeingDragged && ((isPulledFromBottom() && ( (-y + mInitialMotionY) > mTouchSlop )) || (!isPulledFromBottom() && ((y - mInitialMotionY) > mTouchSlop)))) {
    ...
    void onPull(View view, float y) {
        ...
        final float scrollLength = (isPulledFromBottom() ? mPullBeginY - y : y - mPullBeginY);
    ...
    private boolean checkScrollForRefresh(View view) {
        if (mIsBeingDragged && mRefreshOnUp) {
            if (!isPulledFromBottom() && (mLastMotionY - mPullBeginY >= getScrollNeededForRefresh(view))) {
                setRefreshingInt(view, true, true);
                return true;
            }
            if (isPulledFromBottom() && (-mLastMotionY + mPullBeginY >= getScrollNeededForRefresh(view))) {
