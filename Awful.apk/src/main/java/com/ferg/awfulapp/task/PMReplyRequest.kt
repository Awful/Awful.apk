 ...
 */
class PMReplyRequest(context: Context, private val id: Int) : AwfulRequest<Void?>(context, FUNCTION_PRIVATE_MESSAGE) {

    override fun generateUrl(urlBuilder: Uri.Builder?): String = with(urlBuilder!!) {
        appendQueryParameter(PARAM_ACTION, "newmessage")
        appendQueryParameter(PARAM_PRIVATE_MESSAGE_ID, id.toString())
        build().toString()
    }

    override fun handleResponse(doc: Document): Void? {
        // parse the reply data, and create a version that doesn't overwrite the title or reply content, for updating any existing draft
        ...
        // try and update the draft - if it fails, insert the full reply data (including title and message content) as a new draft
